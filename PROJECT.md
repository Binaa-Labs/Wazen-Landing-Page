...
- [ ] OG composite card: sage chip overlaps the region line; EN headline wraps to 4 lines — **pre-existing defect (pre-2.1), fix queued as its own commit** (matters for WhatsApp shares)
- [ ] WhyWazen widens the mobile viewport to ~379px at widths below ~380px (EN) — **pre-existing (Phase 1, found during the 2.1 360px sweep)**, learning-#5 class: the section's grid column min-content is forced wider than the viewport (proof-card address-bar URL is nowrap/LTR; photo slot rides along). Hero/2.1 sections verified clean at 360/390 both locales. Fix queued
